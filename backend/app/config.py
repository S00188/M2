...
class Settings(BaseSettings):
    ...
    # No Neon/Postgres required to run: if DATABASE_URL is missing OR set
    # to an empty string (Render's dashboard sends "" when the field is
    # left blank rather than omitting the var entirely, which would
    # otherwise silently break create_async_engine("")), this falls back
    # to a local SQLite file so the service still boots and the bot still
    # answers. See app/database.py for the production-mode warning this
    # fallback logs, and validate_deployment() below for why this no
    # longer blocks startup in production the way it used to.
    ...
    @classmethod
    def _blank_database_url_falls_back_to_sqlite(cls, v: str | None) -> str:
        if v is None or not str(v).strip():
            return "sqlite+aiosqlite:///./mafia.db"
        return v
    ...
    def validate_deployment(self) -> None:
        # NOTE: DATABASE_URL is deliberately NOT required here. Earlier this
        # raised and killed the whole service at boot (before the webhook
        # was ever registered, so /start got zero response and the reply
        # keyboard never appeared) whenever Neon wasn't configured yet.
        # Now: no DATABASE_URL (or Postgres unreachable) just means the app
        # runs on a local SQLite file instead — see app/database.py for the
        # warning this logs and the tradeoffs of that fallback on Render's
        # free tier (local disk is wiped on every redeploy/restart there).
        if self.environment != "production":
            return
        errors = []
        if len(self.session_secret) < 32 or self.session_secret.startswith("CHANGE_ME"):
            errors.append("SESSION_SECRET: set a random secret of at least 32 characters")
        if ":" not in self.telegram_bot_token or self.telegram_bot_token.startswith("CHANGE_ME"):
            errors.append("TELEGRAM_BOT_TOKEN: set the BotFather token")
        if self.telegram_webhook_enabled and (
            len(self.telegram_webhook_secret) < 16 or self.telegram_webhook_secret.startswith("CHANGE_ME")
        ):
            errors.append("TELEGRAM_WEBHOOK_SECRET: set a random secret")
        if errors:
