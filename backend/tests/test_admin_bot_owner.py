...
async def test_login_response_flags_bot_owner():
    ...
    assert admin_body["is_super_admin"] is True
    assert other_body["is_bot_admin"] is False
    assert other_body["is_super_admin"] is False
...
@pytest.mark.asyncio
async def test_bot_game_respects_the_4_to_20_player_lobby_range():
    """POST /admin/bot-game fills the lobby with the host + bot_count bots,
    so bot_count must land on a 4-20 total: 3..19 bots (never 2 or 20)."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        admin_body = await _login(ac, ADMIN_ID, "Owner")
        headers = {"Authorization": f"Bearer {admin_body['session_token']}"}

        too_small = await ac.post("/admin/bot-game", json={"bot_count": 2}, headers=headers)
        assert too_small.status_code == 400

        too_big = await ac.post("/admin/bot-game", json={"bot_count": 20}, headers=headers)
        assert too_big.status_code == 400

        min_ok = await ac.post("/admin/bot-game", json={"bot_count": 3}, headers=headers)
        assert min_ok.status_code == 200
        assert min_ok.json()["player_count"] == 4

        max_ok = await ac.post("/admin/bot-game", json={"bot_count": 19}, headers=headers)
        assert max_ok.status_code == 200
        assert max_ok.json()["player_count"] == 20


@pytest.mark.asyncio
async def test_admin_stats_reports_active_games():
    """/admin/stats used to duplicate this (see routes_admin.py's removal
    note) — the unified WebApp panel only ever called /admin/statistics,
    so that's the one surface this asserts against now."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        await _make_lobby_game(ac, "-100116")
        admin_body = await _login(ac, ADMIN_ID, "Owner")
        headers = {"Authorization": f"Bearer {admin_body['session_token']}"}
        r = await ac.get("/admin/statistics", headers=headers)
        assert r.status_code == 200
        body = r.json()
        assert body["kp"]["active_games"] >= 1
        assert "total_users" in body["kp"] and "total_finished_games" in body["kp"]
