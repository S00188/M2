...
from app.database import _prepare_asyncpg_url, _pool_kwargs_for
...
def test_pool_pre_ping_and_recycle_enabled_for_neon_style_asyncpg_url():
    """Guards against Neon's free-tier compute auto-suspend: a pooled
    connection from before a suspend must be detected and replaced rather
    than handed out dead."""
    kwargs = _pool_kwargs_for("postgresql+asyncpg://user:pw@ep-x.neon.tech/neondb")
    assert kwargs["pool_pre_ping"] is True
    assert kwargs["pool_recycle"] == 180


def test_pool_pre_ping_not_forced_for_sqlite():
    assert _pool_kwargs_for("sqlite+aiosqlite:///./mafia.db") == {}
