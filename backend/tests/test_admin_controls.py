...
def test_settings_cannot_change_after_game_starts():
    eng, _ = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                      R.CITIZEN, R.CITIZEN])
    with pytest.raises(EngineError):
...
def test_force_advance_morning_to_day():
    eng, ids = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
...
def test_force_advance_lynch_confirmation():
    eng, ids = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
...
def test_force_advance_kamikaze_strike():
    eng, ids = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
...
def test_extend_timer_works_in_morning():
    eng, _ = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                      R.CITIZEN, R.CITIZEN])
    eng.resolve_night_if_ready(force=True)
    assert eng.state.phase == Phase.MORNING
...
def test_extend_timer_works_in_lynch_confirmation():
    eng, ids = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
    PhaseManager.to_lynch_confirmation(eng.state, [ids["P1"]])
    before = eng.state.phase_end
...
def test_extend_timer_works_in_kamikaze_strike():
    eng, _ = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                      R.CITIZEN, R.CITIZEN])
    PhaseManager.to_kamikaze_strike(eng.state)
    before = eng.state.phase_end
...
def test_host_can_kick_player_in_lobby():
    eng = _lobby_engine()
    target = next(pid for pid in eng.state.players if pid != eng.state.host_id)
    eng.kick_player(eng.state.host_id, target)
...
def test_host_cannot_kick_themselves():
    eng = _lobby_engine()
    with pytest.raises(EngineError):
        eng.kick_player(eng.state.host_id, eng.state.host_id)


def test_cannot_kick_after_game_starts():
    eng, ids = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
    with pytest.raises(EngineError):
        eng.kick_player(eng.state.host_id, ids["P1"])
...
def test_admin_remove_mid_game_can_end_the_game():
    eng, ids = make_engine_with_roles([R.CITIZEN, R.MAFIA, R.CITIZEN, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
...
def test_admin_block_reports_night_action_progress():
    eng, ids = make_engine_with_roles([R.MAFIA, R.COMMISSIONER, R.DOCTOR, R.CITIZEN,
                                        R.CITIZEN, R.CITIZEN])
    ...
    assert view["admin"]["night_actions_expected"] == 3


# ---------- bot management ----------

def test_add_bot_and_set_role_in_lobby():
    eng = _lobby_engine(n=5)
    bot_pid = eng.add_bot_player("Bot1")
    assert eng.state.players[bot_pid].is_bot is True
    eng.set_bot_role(eng.state.host_id, bot_pid, "Doctor")
    assert eng.state.players[bot_pid].bot_role_pick == "Doctor"


def test_cannot_add_bot_after_game_starts():
    eng, _ = make_engine_with_roles([R.MAFIA, R.CITIZEN, R.CITIZEN, R.CITIZEN,
                                      R.CITIZEN, R.CITIZEN])
    with pytest.raises(EngineError):
        eng.add_bot_player("Bot1")


def test_non_host_cannot_set_bot_role():
    eng = _lobby_engine(n=5)
    bot_pid = eng.add_bot_player("Bot1")
    other = next(pid for pid in eng.state.players if pid != eng.state.host_id)
    with pytest.raises(EngineError):
        eng.set_bot_role(other, bot_pid, "Doctor")
